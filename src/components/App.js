...
const App = () => {
  ...
  const addItemToCart = (ev) => {
    ...
    if (cart.filter((e) => e.name === newItemName ).length > 0) {
      const updatedCart = cart.map(el => (el.name === newItemName && newItemQuantity) ? {...el, 
        quantity: el.quantity + newItemQuantity}: el)
        console.log(updatedCart)
      setCart(updatedCart);
    }
    else { 
      setCart([...cart, {name: newItemName, price: parseInt(newItemPrice,10), quantity: parseInt(newItemQuantity,10), id: uniqid()   }])
    }
  }
